...
以下要求不是建议、评分偏好或某个 Pilot 的临时结论，而是 Paper2Paper 的产品边界。后续任何文档、脚本、路线推荐和自动校验都不得绕过它们：

- **面向初学者。** AI 必须承担资料搜集、技术判断、路线比较、代码构建和结果解释的主要工作，并用普通语言说明依据、反对理由、不确定性和下一步；不能把一组表格当作作业交给用户自行完成分析。
- **简单模仿路线可以接受并应被认真考虑。** 改动少不构成淘汰理由；原样复现、局部替换、组合替换或其他由锚点框架产生的路线，都要按其具体问题审查，不能按路线类别预先排除。
- **数据必须能够取得。** 正式路线所需数据不仅要有 accession 或网页，还要能够真实下载、解析并具备完成相应 Figure 和 claim 所需的字段；缺失部分及其后果必须明确。
- **代码需要可靠、稳定，缺失时要从同类文献补足。** 不要求锚点作者代码必须可复用；可以组合锚点框架、同类论文、方法论文和官方实现中的可靠代码，但必须记录来源、版本、许可、输入输出、适配、测试和真实运行证据。
- **避免实质重复发表。** 必须检索最接近的已发表工作，并判断具体研究问题、数据、对象、关系、结局和证据链是否实质重合；停止的是具体重复路线，不是“signature”“泛癌”或“简单替换”等整个类别。
- **最终目的是产出论文。** 架构、schema、登记表和回归测试只服务于从锚点文献形成目标论文；一旦某条获批路线能够进入可靠分析和写作，就不得为了继续完善平台而拖延主循环。
- **不能重新引入 PaperRoute 的创新性门槛。** Paper2Paper 不以“创新程度”“综合价值”“变化幅度”“高水平期刊潜力”或其同义替代指标设置准入门槛或暗中排序。是否继续由具体路线的数据、代码、科学设计、Figure 闭环、初学者负担和实质重复风险共同决定。

这些要求也规定了责任边界：AI 先完成可执行的调查和比较，给出明确的首选、备选、反对理由与证据强弱；用户审查并批准高影响决定。可追溯表是审计依据，不是把原始判断工作转交给用户。
...
能力矩阵分别登记参考案例、独立迁移案例、失败案例和成熟度。胃癌 GN-R01 的数据、公式和预期结果只能回归固定 bulk-signature 示例，不能验证其他模态、其他论文或整个 Paper2Paper。
