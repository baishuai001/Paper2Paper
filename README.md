...
Paper2Paper 是一个面向初学者、由 AI 协助执行的论文模仿 workflow。它以真实锚点文献为起点，把论文中的问题、数据、方法、代码和 Figure 拆开审计，再将其中值得借鉴的分析框架转化为一条能够真实执行、根据结果调整并最终形成目标论文的路线。

它的终点是目标论文，不是方向推荐报告，也不是不断扩建的研究平台。“主循环 + 次循环”的架构只是保障论文生产的方法；不能因为架构仍可优化，就推迟已经可以推进的数据、代码、Figure 和稿件工作。
...
主循环回答的是：锚点文献的哪套问题—证据—分析—Figure 框架值得珍惜和利用，怎样把它转化为科学上成立、数据可取、代码可运行、Figure 能闭环且不与现有论文实质重复的目标论文？
...
Paper2Paper 不把模仿理解为从固定菜单中选择一种替换。每篇文献都有自己的研究对象、中心关系、尺度、数据角色、证据顺序、分析模块、Figure 逻辑和未报告选择；路线生成必须先还原这套特有框架，再逐项判断 `retain`、`repair`、`substitute`、`extend` 或 `drop`。这些要素本身也不是封闭 schema，遇到新的论文类型时必须按该论文扩展。

下面只是可能的改写示例，不是完整分类、最低检查表或路线数量要求：
...
指出锚点文献的硬伤不是审计终点，也不自动构成嫌弃或放弃该论文框架的理由。每项硬伤必须同时记录：证据、影响到的 Figure/claim、没有受到影响且仍值得保留的框架、候选修复方案、修复在目标论文中的具体落实、验证或推翻修复的证据，以及修复后的剩余风险和 claim ceiling。

默认顺序是“保留可用框架并修复问题”。只有当中心问题无法形成可证伪主张、所有科学上合理的数据/方法/验证替代均失败，或修复后已经不再保留锚点的中心逻辑时，才把整套框架停止或重定向；即使如此，也要说明哪些局部分析模块仍可借鉴。
