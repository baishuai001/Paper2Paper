...
问题；第 12–13 节明确记录已有 motif 结果之后、因逐行核对作者代码与用户冻结
主问题而发生的等价性修订。任何一节均不得被解释为看结果后放宽阈值。
...
因此，先前合并 47 个模型的一次校正不能再标为“作者等价分析”，而改列为
联合校正敏感性结果。该轮技术诊断中的作者数据库选择路径执行：
...
回到作者的 TF 层数据库选择代码并可能使结果更严格，不是根据结果放宽阈值或
挑选数据库；但它尚未解决完整数据库多重校正和峰到 HOMER 背景构造，后者由
第 13 节重新界定为正式主分析。

## 13. 峰数匹配＋LUSC 背景不得冒充 TNBC 式 motif 主分析

2026-08-17 完成技术等价性审计并得到严格选择路径三端 0 后，用户指出目标论文
需要提出与 TNBC 类似的广泛三系统程序。随后重新打开作者
`Figure2/05B-HOMER.sh`，确认作者峰到 HOMER 的实际命令使用：

- 每个样本自身的完整 peak BED；
- `-size 200`；
- 对全部样本复用 `consensus_TCGA_BRCA_CellLines_MCF7_T47D_background.bed`；
- 没有按最小样本峰数下采样；
- 没有以 non-TNBC 峰作为竞争背景。

前一轮每样本 3,584 峰＋TCGA-LUSC 背景的分析虽然满足“统一峰数、GC、长度、
blacklist 和背景”的技术诊断要求，但同时改变了查询峰深度和生物学参照，回答的
是更严格的 LUAD-vs-LUSC 特异性问题。它从本节起只保留为敏感性分析，不能再
称为 TNBC 作者峰到 HOMER 层的主分析。

用户随后明确冻结新的主分析：全部可用峰统一 200 bp、共同肺癌可及性 consensus
背景、完整 JASPAR/CIS-BP 分库运行、作者 JASPAR 优先规则；峰数匹配与 LUSC
背景均降为敏感性。该变更发生在已有 0/1 结果之后，故必须显式记录；其依据是
作者 05B 代码与研究问题不一致的审计证据，不是放宽 q 值、半数样本规则、HC-TF
集合或根据新结果选择有利分析。

主分析输入冒烟测试在任何新 HOMER 结果产生前进一步确认：MGH1065 的 13 个
`q=0.01` 峰全部位于 random contig，MGH9243 在该阈值下为 0 峰，MGH1157 有
88 个 canonical 峰。三者继续保留在 PDX 固定分母 13 中；MGH1065/MGH9243
显式不可检验，MGH1157 使用全部 88 峰。不以降低 MACS2 q、保留 random contig
或替换样本来制造非空结果。
