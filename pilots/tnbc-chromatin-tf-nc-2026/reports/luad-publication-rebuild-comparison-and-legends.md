...
- 图3分析带符号的regulon、共同/特有的激活靶基因、靶基因群落、TF协作、偏度、与MKI67的相关性，以及肿瘤微环境混杂。图3B与3D现由独立的锚点式渲染器直接复用官方`Figure3/05-Generate_Regulon_Network.R`的网络构建和视觉映射：图3B只显示满足连边规则的实际端点，图3D显示全部正重叠边而不再进行显示性裁边。
...
**A，** 31个HC-TF中每个regulon的激活/抑制及共同/特有靶基因构成。与TNBC官方代码一致，旁侧motif方格标记“在患者、PDX或细胞系中至少一个系统达到原文富集与半数样本规则”的TF；严格三系统共同motif仍作为图2的更强子集单独报告，二者均不作为图3入选条件。**B，** 按锚点文献规则重建的靶基因共同调控图：仅当一对靶基因共同受到至少3个激活HC-TF调控时才连边，并与官方代码一致，只把合格边的实际端点纳入图中。虽然30个激活靶基因各自受到至少3个HC-TF调控，但只有3对靶基因通过配对规则，严格网络因此为6个节点、3条边和3个双节点组分，而不是30个节点加24个孤立点。三个组分分别为NDNF–ADGRF5（共同TF：CSRNP1、ETV1、NR3C2）、ADGRD1–SELENBP1（CRY2、MXD4、ZBTB18）和YPEL3–CACFD1（MAGED2、MXD4、ZNF444）。没有任何组分达到3个基因，故不进行GO富集或赋予功能性群落标签；这一结果不支持“广泛汇聚的靶基因群落”。**C，** 31个HC-TF在TCGA-LUAD肿瘤中的两两活性相关性。**D，** 基于共享激活靶基因构建的完整TF协作网络。按照官方规则，只要一对TF共享至少1个激活靶基因即保留连边；LUAD网络包含31个节点和134条边，构成1个连通分量且没有孤立节点。节点大小表示TF网络连接数，边宽表示共享激活靶基因数，不再使用80%边权分位数进行显示性裁边。**E，** regulon广度与协作TF数量之间的关系。**F，** TCGA-LUAD中HC-TF活性的偏度，并采用BH校正后的偏度推断。**G，** 代表性活性分布，分别展示正偏、负偏及近似对称的调控状态。
...
| 3A | 量化带符号的regulon及其特有/共同靶基因，并标记至少一个系统达到motif规则的TF。 | 量化31个HC-TF的regulon；motif方格按患者、PDX或细胞系任一系统达到原文规则定义。 | 证据结构等价但规模更小；严格三系统共同motif仍留在图2单独报告，避免把“任一系统motif支持”“三系统共同motif”和“HC-TF入选”混为一谈。 |
| 3B | 揭示由共同激活HC-TF连接的靶基因群落。 | 按官方规则从激活共同靶基因出发，仅保留共同拥有至少3个激活HC-TF的靶基因对；严格网络为6个实际端点、3条边和3个双节点组分。 | 原文没有独立的“把所有受到≥3个TF调控的靶基因都画成节点”规则。30个此类候选中有24个没有合格配对边，应保留在数据表而不是散布为网络孤点。主图不降低阈值，也不对双节点组分强作GO解释；紧凑dyad版明确列出每对靶基因及其3个共同TF。 |
| 3C | 展示HC-TF共同活性结构。 | 展示TCGA肿瘤中的LUAD TF共同活性。 | LUAD矩阵更小；应保持正方形、完成聚类，并确保最终版面下仍可读。 |
| 3D | 通过共享激活靶基因展示TF协作。 | 按官方最小共享数1显示31个HC-TF的全部134条正重叠边。 | 完整LUAD网络是单一连通分量且没有孤立点；旧图中的10个孤立点来自额外的80%边权分位数裁边，并非生物学稀疏性。修订版取消裁边，以连接数控制节点大小、共享靶基因数控制边宽，使用统一暗红节点、灰色半透明边、repel标签和双图例。 |
