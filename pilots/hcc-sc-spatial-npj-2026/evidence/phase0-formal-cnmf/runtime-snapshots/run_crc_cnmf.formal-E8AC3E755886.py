...
def validate_counts_input(path: Path) -> dict[str, object]:
    ...
    try:
        ...
        if "cna_selection_class" in data.obs.columns:
            ...
            expected = ["author_cancer_two_method_malignancy_support"]
            if selection_classes != expected:
                raise ValueError(
                    "cNMF CNA selection contains unexpected evidence classes: "
                    f"{selection_classes}"
                )
            selection_basis = "copykat_aneuploid_and_scevan_tumor_intersection"
...
def run_cnmf(
    ...
    if run_role != "diagnostic" and run_role not in FORMAL_RUN_ROLES:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-role",
        choices=["diagnostic", *sorted(FORMAL_RUN_ROLES)],
        default="diagnostic",
    )
