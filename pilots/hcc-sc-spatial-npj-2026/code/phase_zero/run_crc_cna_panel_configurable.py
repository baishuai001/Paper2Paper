...
def run_panel(
    ...
    limits = {
        "max_cancer": max_cancer,
        "max_epithelial": max_epithelial,
        "max_reference": max_reference,
        "min_cancer": min_cancer,
        "min_reference": min_reference,
    }
    if any(value < 0 for value in limits.values()):
        raise ValueError(f"CNA cell limits must be nonnegative: {limits}")
    if max_cancer < min_cancer or max_reference < min_reference:
        raise ValueError(f"CNA maxima must not be below minima: {limits}")
    if reference_labels is not None and not reference_labels:
        raise ValueError("explicit CNA reference-label list must not be empty")
    ...
    state: dict[str, object] = {
        ...
        "seed": seed,
        "cell_limits": limits,
        "reference_labels": reference_labels or "prepare_crc_cna_inputs.DEFAULT_REFERENCE_LABELS",
        "units": [],
    }
