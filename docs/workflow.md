...
退出条件：用户能够理解每幅主图在证据链中的作用，且不可公开复现的边界已写明。
...
- 至少检查原样复现、marker、基因集、细胞、癌种、泛癌、signature 和合理组合；
...
退出条件：常见路线没有被无理由遗漏，且每条 active 路线都能用普通语言说明。
...
目标：让用户审查文献侧结果和产品侧 finding，而不是只看一个 AI 推荐。
