...
- 对检出的每篇近邻论文先做 donor 分解：分别记录它能提供的科学框架、数据、代码、方法、Figure 角色和生物学对象；另行记录它已经占据的具体疾病—对象—关系—结局—证据链。二者不能合并成一个简单的 `stop` 判断；
- 建立“锚点框架资产 × donor 资产 × 开放候选池”的组合空间，再把能形成同一中心问题和 Figure 闭环的组合写成路线。近邻论文的字面主线已经发表，只禁止原封不动再做；其分析框架、公开数据和可靠代码仍应与锚点组合；
- 对任何准备比较的改变轴，记录不止一个有依据的候选，或者明确说明为什么当前只能找到一个以及下一步怎样补搜。单个细胞状态、单个癌种或单篇近邻论文不能证明整条轴已覆盖；
...
退出条件：锚点特有框架中的重要可变要素没有被无理由遗漏；重要近邻论文已经完成 donor/重复风险双重拆解；候选轴没有被单个例子冒充完整覆盖；且每条 candidate 路线都能说明“从锚点文献保留了什么、从哪些 donor 借了什么、修复/改变了什么、怎样形成目标论文”。此阶段只形成候选池，不允许依据未经核查的印象提前指定首选。
