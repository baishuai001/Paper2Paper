...
- 将路线用途标记为稿件候选、限定模块的训练实例或辅助分析；用途、执行状态与稿件资格不得混为一谈；
...
- 下载并解析代表性数据文件；
- 把每个必需字段映射到实际数据资源并核对跨来源ID；
- 登记每个队列在筛选、拟合、cutoff和验证中的用途；
- 核对疾病、组织、模态、统计单位、必需字段和最低受试者数；
- 安装代码并运行真实输入的非交互smoke test；
- 在目标OS、用户权限和路径环境中完成smoke test；
- 对signature锁定可计算模型合同；
- 保存smoke input、output和必需测试证据；
- 映射最低主图并至少生成一个代表性输出；
- 检查最近发表和实质重复。
- 将运行中发现的问题写入`issues.tsv`并决定实例处理或推广到核心。

退出条件：至少一条路线没有未解决的执行缺口；若要进入路线选择，它还必须具备稿件资格。
...
- `PROJECT.json.selected_route_id`只能指向一条既执行就绪又具备稿件资格的路线。
- 训练复现和辅助分析即使已经跑通，也不能被选为最终稿件路线。
...
`paper2paper validate`只检查结构、登记合同和关键跨阶段依赖；它不会重新运行真实数据，
也不能替代用户检查科学内容真实性。
